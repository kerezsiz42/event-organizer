...
class VoteController {
    ...
    ResponseEntity<VoteOutput> putVote(@Valid @RequestBody final VoteInput v,
        ...
        if (poll.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("% not found", v.getPoll()));
        }
        Optional<Option> option = optionRepository.findById(v.getOption());
        if (option.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, String.format("% not found", v.getOption()));
        }
